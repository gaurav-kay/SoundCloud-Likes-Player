...
# un-shuffled
shuffle(links_list)  # shuffle w/ return type returns None. https://stackoverflow.com/questions/17649875/why-does-random-shuffle-return-none
# shuffled

driver = webdriver.Chrome('D:/Download/chromedriver_win32/chromedriver.exe')
driver.implicitly_wait(5)  # keeps checking 2 times in 1 second if the DOM element has loaded for 5 seconds
...
for current_link in links_list:
    ...
    duration = driver.find_element_by_css_selector('#app > div.playControls.g-z-index-control-bar.m-visible > div > div > div.playControls__elements > div.playControls__timeline > div > div.playbackTimeline__duration > span:nth-child(2)')
    # duration in the lower playback controller

    print("Track length = ", duration.text)  # log
    duration = str(duration.text).split(':')  # duration.text is in the form minutes:seconds
    ...
    if driver.find_element_by_css_selector('.playControl').text != 'Pause current':
        ...
        sleep(minutes * 60 + seconds - 10)  # 1st window takes more than required time hence - 10
